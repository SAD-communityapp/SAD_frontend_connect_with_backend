...
const Marking = (props) => {
    ...
    const renderPeriod = (index, item) => {
        const { color, startingDay, endingDay, text, betweenStartAndEnd} = item;
        let textColor = colors.text_white; 
        const styles = [
        ...
        if (endingDay) {
            textColor = colors.primary_100
            styles.push(style.current.endingDay);
        }
        if (!startingDay && !endingDay && betweenStartAndEnd) {
            textColor = colors.primary_100;
            styles.push(style.current.period); // 或者根据需要使用其他样式
        }
        return(
            // <Text
